...
public function setDateWhere(&$where, $start, $end, $field){
        if ($start && false !== strtotime($start)) {
            if ($end && false !== strtotime($end)) {
                $end .= ' 23:59:59';
                $where[$field] = [ ['>=', strtotime($start)], ['<=', strtotime($end)], 'and'];
            }else{
                $where[$field] = [['>=', strtotime($start)], ['<=', TIME_NOW], 'and'];
            }
        }else{
            if ($end && false !== strtotime($end)) {
                $end .= ' 23:59:59';
                $where[$field] = [ ['>=', 0], ['<=', strtotime($end)], 'and'];
            }
        }
    }
    ...
    public function getIndexData()
    {
        ...
        // 产品信息
        $data['product_name']   = '善策';
        $data['author']         = 'Bigotry';
        $data['website']        = 'www.onebase.org';
        $data['qun']            = '<a target="_blank" href="//shang.qq.com/wpa/qunwpa?idkey=3807aa892b97015a8e016778909dee8f23bbd54a4305d827482eda88fcc55b5e"><img border="0" src="//pub.idqqimg.com/wpa/images/group.png" alt="善策 ①" title="善策 ①"></a>';
