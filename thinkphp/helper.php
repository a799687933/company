...
function feeratio($arr){
     $fee_rs =Db::name('fee')->field($arr)->find(); 
      $shuzu = explode("|",$fee_rs[$arr]);//如果有执行这个遍历
              foreach ($shuzu as $key => $valu) {//取出对应规则
               if ($arr1!=NULL) { unset( $shuzu);}
               $arr2= explode('-',$valu);
                if($arr2[1]!=NULL){
                $shu[$arr2[0]]=$arr2[1]; unset($shuzu);
                 }       
              }
         if($shu){
              return  $shu;}
